...
func (h *HTTPResponseHandler) ErrorResponse(err error, msg string) {
	...
	case errors.Is(err, core_errors.ErrUserAlreadyExists):
		statusCode = http.StatusBadRequest
		logFunc = h.log.Warn

	case errors.Is(err, core_errors.ErrInvalidAuthData):
		statusCode = http.StatusBadRequest
		logFunc = h.log.Warn
	...
	case errors.Is(err, core_errors.ErrGenerateToken):
		statusCode = http.StatusInternalServerError
		logFunc = h.log.Error

	case errors.Is(err, core_errors.ErrConnectDB):
		statusCode = http.StatusInternalServerError
		logFunc = h.log.Error
